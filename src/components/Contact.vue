...
<template>
  <section
    id="contact"
    class="py-8 bg-gradient-to-b from-gray-50 to-indigo-50 px-8 md:px-20 dark:from-slate-900 dark:to-slate-900"
  >
    <h1
      data-aos="fade-up"
    ...
    <section class="w-full text-indigo-700 text-xl gap-5 mt-5">
      <div class="flex w-full gap-3 md:w-7/12 md:mx-auto lg:justify-center">
        <template v-for="(contact, x) in contacts" :key="x">
          <a
            data-aos="zoom-in"
            :href="!contact.mailto ? contact.link : 'mailto:' + contact.link"
      ...
      <!-- <form
        ...
        class="lg:w-10/12 lg:mx-auto mt-8 md:flex items-start gap-5 justify-between"
      >
        ...
        <section class="w-full md:w-5/12 flex flex-wrap">
          <label data-aos="fade-up" class="text-base dark:text-indigo-400"
            >Message</label
          >
          <textarea
            data-aos="fade-up"
            rows="13"
            name="message"
            class="w-full text-base text-gray-600 mt-2 p-2 border-2 border-indigo-300 rounded-xl overflow-hidden bg-gray-50 dark:bg-slate-200"
            placeholder="Your message"
          ></textarea>
          <div data-aos="fade-up" class="w-full mt-5 flex justify-between">
            <button
              :class="isLoading ? 'bg-blue-200' : 'bg-blue-500'"
              class="text-gray-100 w-5/12 py-2 rounded-lg text-base"
              type="submit"
            >
              <i
                v-if="statusMsg !== 'Failed'"
                :class="statusMsg === 'Ok' ? 'fa-check' : 'fa-paper-plane'"
                class="fa mr-1"
              ></i>
            ...
            <button
              @click="statusMsg = 'Send'"
              class="border-red-500 border-2 text-red-600 w-5/12 py-2 rounded-lg text-base"
              type="reset"
            >
...
const URL =
  "https://script.google.com/macros/s/AKfycbwDLoFe7LvF7-UkfCC6BNE68ORGJnkMdcCiwTVGOqlacDhh02R4abuOv9Rth-IqOrbcgQ/exec";
